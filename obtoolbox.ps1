...
"@ -ForegroundColor DarkCyan
    ...
    switch ($choice) {
        ...
        "19" { Invoke-NetScan }
        "20" {
            Start-Process powershell.exe -ArgumentList "-NoProfile -WindowStyle Hidden -Command & {Import-Module '$env:TEMP\obsoftware\ob.psm1'; Get-IPScanner; pause}" -Verb RunAs
            Read-Host "Advanced IP Scanner is downloading in the background and will start in a moment. " 
        }
        "21" {
            Start-Job -ScriptBlock { Import-Module "$env:TEMP\obsoftware\ob.psm1"; Invoke-WizTree; Exit } > $null
            Read-Host "WizTree is downloading in the background and will start in a moment"
        }
        "22" {
            Start-Job -ScriptBlock { Import-Module "$env:TEMP\obsoftware\ob.psm1"; Invoke-DotNetRepair; Exit } > $null
            Read-Host ".NET Repair tool is downloading in the background and will start in a moment"
        }
        "23" {
            Start-Job -ScriptBlock { Import-Module "$env:TEMP\obsoftware\ob.psm1"; Invoke-HWInfo; Exit } > $null
            Read-Host "HWiNFO is downloading in the background and will start in a moment"
        }
        "24" {
            Start-Job -ScriptBlock { Import-Module "$env:TEMP\obsoftware\ob.psm1"; Invoke-TeamViewerQS; Exit } > $null 
            Read-Host "TeamViewerQS is downloading in the background and will start in a moment"
        }
        "25" {
            Start-Job -ScriptBlock { Import-Module "$env:TEMP\obsoftware\ob.psm1"; Invoke-RevoUninstaller; Exit } > $null 
        ...
        "00" {
            Remove-Item -Recurse -Force "$env:TEMP\obsoftware"
